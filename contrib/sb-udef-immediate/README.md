...
one of the tags is reserved as `UDEF-IMMEDIATE-WIDETAG`:
...
(sb-udef-immediate:def-udef-immediate probability
                                  :max-bits +prob-bits+
...
For dynamically-sized data, see _Buffer indexing_ below.
...
:mode :subseq)` to get a fresh copy. (This makes a difference for GC and 
...
There are two main modes of using column-structures:
...
allocated storage vector, which takes CPU time --
and concurrent write accesses might get lost.
...
can never return a displaced vector into the storage array - so a plain
`(setf (aref (person-siblings *somebody*) 0) ...)` will get lost; for that please see 
...
```lisp
(sb-udef-immediate:def-bitfield-struct bbbits
...
This is used in _Buffer indexing_, see below.


With immediate slots only no actual storage vectors are allocated.

Please be aware that these "immediate" slots are kept in the _udef_, which is an 
immediate value and so *not passed by reference* into called functions! \
To avoid confusion at runtime,
the setter functions for immediate slots require acknowledging this via an 
additional `:IMMEDIATE` positional argument.
...
storage vector in a single _udef immediate_ value means that only a few specialized arrays 
...
2²⁴ characters, all required data can be packed into a 32bit value; by using 
...
and the index points into large vectors of 2²⁰ characters each.

When storing such a `filename` in a typed slot in another C-S, only 
`(unsigned-byte 32)` storage vectors for the actually used bits are allocated.

If you need such conversions somewhere else (eg. for normal CLOS classes or structures), you might find
`make-wrapped-udef-accessor` helpful.


## Iteration

If you need to iterate over the _allocated_ values of a C-S,
`C-S-VALUES` returns a list of udef immediates; `MAP-C-S-RANGE`
runs a lambda for each value and collects the return values.

Both are suitable only for plain column-structures without immediate slots --
ie. where the immediate stores (apart from the tag bits) only an index into vectors!
