...
#. (let ((what (or (second sb-ext:*posix-argv*)
     ...
     (defparameter *def* (if (string= what "1")
                             'defstruct
                             'sb-udef-immediate:def-column-struct))
...
(defun check ()
  ...
  (loop with instance = *data*
        ...
        do (let ((type (sb-udef-immediate:udef-immediate-type-of instance))
                 (raw  (sb-udef-immediate:udef-general-get-value instance)))
...
(defun sizes ()
  (if (eq *def* 'defstruct)
    ...
    (format t "      Used items: ~d of ~d~%"
            (sb-udef-immediate:column-struct-last-index 'my-integers)
            (sb-udef-immediate:column-struct-size 'my-integers))))
...
;; The previous widetag #x61 broke S-L-A-D when a UDEF was stored in a CONS;
;; make sure this keeps working now.
(defparameter *udef-cons* (cons *data* *data*))
(defparameter *udef-vector* (vector *data*))
