...
(defun column-struct-get-struct (obj &key (output :alist))
  ...
  (check-type obj sb-int:udef-inttype)
  (multiple-value-bind (type) (udef-inttype-type-of obj)
         ...
         (let* ((2-level? (cs-meta-batch-size c-s))
           ...
           (loop for slot-def across (cs-meta-slots c-s)
                 ...
                 do (let* ((slot-nr (cs-s-index slot-def))
                           ...
                           (old-count (/ (array-dimension slot-vec 0)
                                               (if 2-level?
                                                   1
                                                   elem-per-slot)))
                           (new-elements (- new-vec-size old-count)))
                      (setf (aref (cs-meta-data-vec c-s)
                            ...
                            (if 2-level?
                                ...
                                ;; 1-level reallocation
                                (let ((new (make-array (* elem-per-slot
                                                          new-vec-size)
                                                       :element-type slot-type
                                                       :initial-element (single-initial-element slot-def))))
                                  (when (plusp old-count)
                                    ;; Populate with old data.
                                    (replace new slot-vec
                                             :end1 (length slot-vec)))
                                  new)))))
           (sb-vm:%write-barrier)
           (setf (cs-meta-allocated c-s)
                 (if 2-level?
         ...
         `(let ((,',next 0)
            ...
            (labels ((,',new-batch ()
                     ...
                     (,fn-name (&key ,@', (rest arg-list))
                       (let* ((id ,',next)
                              (needed ,',(or (cs-s-slot-name
                                               (cs-meta-var-len-slot
                                                 (get-cs-metadata-from-symbol
                                                   col-struct)))
                                             1)))
                         ;; TODO: needs to check availability in current batch,
                         ;; not just reserved number range!
                         (when (/= 1 needed)
...
(defmethod possibly-udef-value-translation ((udef udef-metadata))
  (values (udef-metadata-func udef)
          udef))
  ...
  (multiple-value-bind (func) (possibly-udef-value-translation slot)
    (with-slots (accessor-sym slot-name) slot
      `((,accessor-sym (id)
        ...
        ((setf ,accessor-sym) (new place immediate!)
           (declare (optimize (speed 3) (safety 1) (debug 1))
    ...
    (multiple-value-bind (func translate?) (possibly-udef-value-translation slot)
      (flet
        ...
        `((,accessor-sym (id &key start end index (mode :displaced))
           ...
           and :MODE :SUBSEQ for a separate sequence."
  ...
  (multiple-value-bind (func) (possibly-udef-value-translation slot)
    (with-slots (accessor-sym slot-name) slot
      `((,accessor-sym (id)
        ...
        ((sb-ext:cas ,accessor-sym) (old new id)
           ,(setup-slot-vector-addressing
              (get-cs-metadata-from-symbol *current-cs-sym*)
              slot 'id 'vec 'idx
...
(defun get-slot-def (input)
  (multiple-value-bind (name init user-type allocation) (parse-slot input)
    (multiple-value-bind (c-s meta) (get-cs-metadata-from-symbol user-type)
      ;; Can be an immediate or vector storage slot
      (when (eq user-type *current-cs-sym*)
        ;; TODO: can be dropped?
        (setf meta :current))
              ...
              ((eq allocation :immediate)
               (multiple-value-bind (bits)
                   (cond
                     ...
                     ((eq meta :current)
                      (or *current-cs-size*
                          (error "Self-referencing UDEFs need to specify :INDEX-BITS")))
                     (meta
...
(defun check-var-len-slots (slot-defs)
  ...
  (let ((var-len-args ())
        (non-imm-slots ()))
    (dolist (slot slot-defs)
      (cond
        ((slot-def-has-some-mixin? slot 'cs-mixin-variable-sized-array)
         (pushnew (second ; the symbol is quoted!
                    (getf (nthcdr 2 slot)
                          :array-len-var))
                  var-len-args))
        ((slot-def-has-some-mixin? slot 'cs-mixin-immediate)
         t)
        (t
         (push (second
                 (getf (nthcdr 2 slot)
            ...
            (cond
              ...
              ((typep ,udef-input ',(cs-s-storage-type cs))
               ...
               (ldb ,(destructuring-bind (typ l s) (cs-s-ldb slot)
                       (list typ l (- s +udef-reserved-low-bits+)))
                    (sb-kernel:get-lisp-obj-address ,udef-input))))
            ((setf ,fn) (new)
  ...
  `(,udef-func :int-to-tagged-udef
               (logior
                 ,@ (loop for slot across slots
                          for var-name = (cs-s-slot-name slot)
                          for translator = (possibly-udef-value-translation slot)
                          when (typep slot 'cs-mixin-immediate)
                          ;; Only add the index value of an udef, not any other immediate slots!
                          ;; So mask to the actually allocated bits,
                          ;; this is identical to the number of index bits of the referenced udef.
                          collect `(ash (ldb (byte ,(cs-s-bit-count slot) 0)
                                             (,translator :udef-or-nil-to-ub-x ,var-name))
                                        ,(- (cs-s-bit-start slot)
                                            +udef-reserved-low-bits+))))))

(defmacro bit-fixup-macro ()
  ;; Only serves to keep that block out of the LET below
  `(progn
     (cond
  ...
  (destructuring-bind (struct-name &rest options) (sb-int:ensure-list name-and-options)
    ...
    (flet ((option (name default)
      ...
      (let* ((*current-cs-sym* struct-name)
             ...
             (immediate-bits (apply #'+
                                    (mapcar (lambda (s)
                                              (if (slot-def-has-some-mixin? s 'cs-mixin-immediate)
                                                (getf (nthcdr 2 s) :count)
                                                  0))
                                            user-slots)))
             ;;
      ...
      (loop for slot across slots
            ...
            (return
              (sb-int:with-unique-names (up-to old-size len tmp numeric-index where)
                `(progn
                   ;; Accessors
                   ;(declaim (inline ,(mapcar #'first accessors)))
                   ,@ (mapcar (lambda (form)
                                (cons 'defun form))
                              accessors)
                   ;;
                   ...
                   (defun ,base-constructor ,slot-names
                     ...
                     (let* ((,where (load-time-col-struct ',*current-cs-sym*))
                       ...
                       ;; TODO: drop the resize checks for immediate-only structures
                           ...
                           (let* ((,tmp (get-cs-metadata-from-symbol ',*current-cs-sym*))
                                  (,numeric-index (get-new-id-range ,tmp
                                                                    ,(cs-s-slot-name
                                                                       (cs-meta-var-len-slot cs))
                                                                    t))
                                  (max ,(expt 2 (cs-meta-index-bits cs))))
              ...
              (multiple-value-bind (flags req opt rest keys)
                ...
                `(defun (setf ,new) ,old-args
                   (setf (,old ,@ (cdr req)
                               ,@ opt
                               ,@ rest)
                         (,(udef-metadata-from-udef udef) ,(first req)))
                   ,(first req)))))
         ',new))))
