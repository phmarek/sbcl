...
(assert (not (sb-udef-immediate:udef-inttype-type-of 4)))
...
(with-test (:name (:udef :storage-vector-types))
  ...
  (flet ((v-type (c-s slot)
           (let* ((cs (sb-udef-immediate::get-cs-metadata-from-symbol c-s)))
             (sb-udef-immediate::cs-s-storage-type
               (find slot (sb-udef-immediate::cs-meta-slots cs)
                     :key #'sb-udef-immediate::cs-s-slot-name)))))
    ;;
